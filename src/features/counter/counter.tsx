...
import { funcDecrement, funcIncrement, funcIncrementByAmount } from './counterSlice'
...
// Without Saga
  ...
  // With Saga
  const counterSaga = useSelector((state: RootState) => state.rootReducer.apireducer.value)
 ...
 let dataLength = 0;
  ...
  //  if(allData.rootReducer.apireducer.apiname === 'testapi'){
    temp = useSelector((state : RootState) => state.rootReducer.apireducer ? state.rootReducer.apireducer.apiData : null);
   // listData = useSelector((state : RootState) => state.rootReducer.apireducer ? state.rootReducer.apireducer.apiData : null);
  ...
  return (
    <View style = {{flex: 1, alignItems : 'center', justifyContent : 'center'}}>
      <View style = {{}}>
        <TouchableOpacity
        style = {{marginTop : 0, backgroundColor : 'orange', padding : 10,
        alignItems : 'center', justifyContent : 'center'}}
          onPress={() => dispatch(funcIncrement())}
        >
          <Text>Increment Without MiddleWare</Text>
        </TouchableOpacity>
        <Text style ={{color :'red', marginTop : 20}}>Count without Middleware :  {count}</Text>
        <TouchableOpacity
      ...
      <Text style = {{marginTop : 20,
         fontWeight : 'bold',
         fontSize : 14}}>Data Getting from API</Text>
        <FlatList
        style = {{marginTop : 20}}
        data={listData || []} 
      ...
      <TouchableOpacity
        ...
        <Text style ={{color :'red', marginTop : 20}}>Counter with Saga :  {counterSaga}</Text>
        <TouchableOpacity
        style = {{marginTop : 20, backgroundColor : 'orange', padding : 10,
        alignItems : 'center', justifyContent : 'center'}}
